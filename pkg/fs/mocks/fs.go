...
// BulkWrite provides a mock function with given fields: writes
func (_m *FS) BulkWrite(writes ...fs.BulkWriteRequest) error {
	_va := make([]interface{}, len(writes))
	for _i := range writes {
		_va[_i] = writes[_i]
	}
	var _ca []interface{}
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(...fs.BulkWriteRequest) error); ok {
		r0 = rf(writes...)
	} else {
		r0 = ret.Error(0)
	}
